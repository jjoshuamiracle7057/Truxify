...
async function getBookingContext(bookingId, userId) {
  ...
  // Orders table is the real order model
  // (there is no bookings table).
  try {
    let orderQuery = voiceDb
      .from('orders')
      .select('*');
...
function buildResponseForIntent(intent, bookingData, transcript) {
  const orderRef =
    bookingData?.order_display_id ||
    bookingData?.id ||
    'your order';

  const status =
    bookingData?.status ||
    bookingData?.current_status ||
    'in transit';

  switch (intent) {
    case 'payment_status':
      return `Your payment for ${orderRef} is ${
        bookingData?.escrow_status || 'processing'
      }.`;
...
export async function processVoiceQuery(
  ...
  const bookingData =
    await getBookingContext(bookingId, userId);
  ...
  if (
    ...
    logger.warn(
      'Missing OpenAI or ElevenLabs API keys. Using mock Voice AI intent pipeline.'
    );

    let transcript =
      textQuery || 'Where is my package?';

    if (!textQuery && audioBuffer) {
      // Deterministically sample query based on
      // buffer byte content or intent matching
      const querySamples = [
        'Where is my package?',
        'When will it arrive?',
      ...
      const byteSum = audioBuffer.reduce(
        (acc, val) => acc + val,
        0
      );

      transcript =
        querySamples[byteSum % querySamples.length];
    }

    const intent =
      detectQueryIntent(transcript);
  ...
  try {
    ...
    const header =
      ...
      `name="file"; filename="${filename || 'audio.wav'}"\r\n` +
  ...
  try {
    const systemPrompt =
      `You are Truxify Voice AI Assistant for freight tracking. ` +
      `Answer in 1-2 concise sentences in the customer's language ` +
      `(English/Hindi/Tamil). Focus on intent: ${intent}.\n` +
...
### The actual Issue #14333 fix

The only change needed for that issue is this:
...
* `bookingId = null` → returns `null`
* `bookingId = undefined` → returns `null`
* valid UUID → continues normally
* non-UUID booking ID → continues with the existing logic

So **don't make additional unrelated changes** when submitting the PR for this issue. Keeping the PR focused on the requested null guard is preferable.
