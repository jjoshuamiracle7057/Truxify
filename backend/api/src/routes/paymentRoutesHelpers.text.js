...
import {
  loadFilterQuerySchema,
  createLoadSchema
} from '../validation/loadSchemas.js';
import {
  validateBody,
  validateParams,
  validateQuery
...
router.get(
  ...
  validateQuery(loadFilterQuerySchema),
  async (req, res) => {
    try {
      ...
      // Strict validation for pagination values.
      ...
      // Handle vehicle_type filtering in JS.
      if (
        req.query.vehicle_type &&
        typeof req.query.vehicle_type !== 'string'
      ) {
        return res.status(400).json({
          error: 'vehicle_type must be a single string'
        });
      }
      ...
      if (req.query.status) {
        if (typeof req.query.status !== 'string') {
          return res.status(400).json({
            error: 'status must be a single string, not an array or object'
          });
        }
        ...
        if (
          ...
          if (allowedStatuses.includes(statusLower)) {
            ...
            return res.status(400).json({
              error:
                'status must be one of: open, available, claimed, expired, cancelled'
            });
          }
        }
      }
      ...
      if (req.query.pickup_location) {
        const pickupLocation = (
          Array.isArray(req.query.pickup_location)
            ? req.query.pickup_location[0]
            : req.query.pickup_location
        ).trim();

        if (!pickupLocation) {
          return res.status(400).json({
            error: 'pickup_location must not be empty'
          });
        }

        if (pickupLocation.length > 200) {
          return res.status(400).json({
            error: 'pickup_location too long (max 200 chars)'
          });
        }

        query = query.ilike(
          'pickup_address',
          `%${escapeLike(pickupLocation)}%`
        );
      }
      ...
      if (req.query.destination) {
        if (Array.isArray(req.query.destination)) {
          return res.status(400).json({
            error: 'Repeated destination parameters are not allowed'
          });
        }
        ...
        if (destination.length > 200) {
          return res.status(400).json({
            error: 'destination too long (max 200 chars)'
          });
        }
      ...
      if (req.query.goods_type) {
        if (typeof req.query.goods_type !== 'string') {
          return res.status(400).json({
            error: 'goods_type must be a single string'
          });
        }
        ...
        if (!goodsType) {
          return res.status(400).json({
            error: 'goods_type must not be empty'
          });
        }
      ...
      if (filters.min_price !== undefined) {
        query = query.gte(
          'freight_value',
          Math.round(filters.min_price * 100)
        );
      }
      ...
      if (filters.max_price !== undefined) {
        query = query.lte(
          'freight_value',
          Math.round(filters.max_price * 100)
        );
      }
      ...
      if (filters.distance !== undefined) {
        query = query.or(
          `extra_distance_km.is.null,extra_distance_km.lte.${filters.distance}`
        );
      }
      ...
      const formattedLoads = (loads || []).map((load) => ({
        ...
        estimated_price: load.freight_value / 100,
        vehicle_type: 'Truck'
      }));
      ...
      logger.error(
        ...
        'Internal Server Error in GET /api/loads'
      );
    ...
    try {
      ...
      const { data, error } = await supabaseAdmin
        ...
        .insert({
          ...
          freight_value: Math.round(
            parseFloat(expected_price) * 100
          ),
      ...
      if (error) {
        logger.error(
          ...
          'Failed to create load offer'
        );

        return res.status(500).json({
          error: 'Failed to create load offer',
          details: error?.message
        });
      }

      invalidateBookingCaches().catch((err) => {
        logger.error(
          {
            error: err?.message
          },
          'Failed to invalidate cache on load creation'
        );
      });
      ...
      logger.error(
        ...
        'Internal Server Error in POST /api/loads'
      );
...
router.get(
  ...
  async (req, res) => {
    ...
    if (
      ...
      return res.status(400).json({
        error: 'vehicleType must be a string'
      });
    }
    ...
    const cacheKey =
      `loads:stats:${normalizedVehicle ? normalizedVehicle.toLowerCase() : 'all'}`;
    ...
    if (redisClient) {
      try {
        ...
        logger.warn(
          ...
          'Failed to read load stats from Redis cache'
        );
      }
    }
    ...
    try {
      ...
      if (!statsData) {
        const query = supabaseAdmin
          .from('load_offers')
          .select(
            'freight_value, extra_distance_km'
          )
          .eq('status', 'available');
        ...
        if (error) {
          logger.error(
            ...
            'Failed to fetch load stats from DB'
          );
        ...
        for (const row of rows || []) {
          const valInr =
            (Number(row.freight_value) || 0) / 100;

          sumFreight += valInr;

          if (valInr < minFreight) {
            minFreight = valInr;
          }

          if (valInr > maxFreight) {
            maxFreight = valInr;
          }

          if (
            row.extra_distance_km !== null &&
            row.extra_distance_km !== undefined
          ) {
            const dist =
              Number(row.extra_distance_km) || 0;
        ...
        statsData = {
          vehicleType:
            normalizedVehicle || 'all',
          activeLoads,
          avgFreightPrice:
            activeLoads > 0
              ? Number(
                  (
                    sumFreight / activeLoads
                  ).toFixed(2)
                )
              : 0,
          minFreightPrice:
      ...
      if (redisClient && statsData) {
        try {
          ...
          logger.warn(
            ...
            'Failed to cache load stats in Redis'
          );
        }
      }
      ...
      logger.error(
        ...
        'Internal Server Error in GET /api/loads/stats'
      );
    ...
    try {
      ...
      if (error) {
        logger.error(
          ...
          'Failed to fetch load offer by ID'
        );

        return res.status(500).json({
          error: 'Failed to fetch load offer.'
        });
      }
      ...
      if (!load) {
        return res.status(404).json({
          error:
            'Load offer not found or no longer available.'
        });
      }
      ...
      logger.error(
        ...
        'Internal Server Error in GET /api/loads/:id'
      );
